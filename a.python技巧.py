...
技巧：   1.使用 nums[right] - nums[left] <= 2 * k：   （力扣2779）
            当前滑动窗口内的最大值和最小值的差距小于或等于 2 * k 时，窗口内的所有元素都可以通过调整变为相同的值，可以继续扩展窗口。
                例子：nums = [1, 5, 9], k = 2
                    排序后的数组：[1, 5, 9]。
                    对于每个元素：
                        1 可以变为 [1 - 2, 1 + 2] = [-1, 3]。
                    ...
                    判断窗口 [1, 5, 9] 的条件：
                        nums[right] - nums[left] = 9 - 1 = 8。
                        8 大于 2 * 2 = 4，不满足条件。
                        结果：无法通过调整让它们相等，所以我们需要缩小窗口。

        2.使用 nums[right] - nums[left] > 2 * k：
            当前滑动窗口内的最大值和最小值的差距大于 2 * k 时，窗口内的元素无法通过调整变为相同的值，需要缩小窗口，移动 left。
                例子：nums = [1, 5, 9], k = 2
                    排序后的数组：[1, 5, 9]。
                    当前窗口是 [1, 5, 9]，即 nums[left] = 1 和 nums[right] = 9
                        它们的差距是 9 - 1 = 8，大于 2 * 2 = 4，无法满足条件。
'''